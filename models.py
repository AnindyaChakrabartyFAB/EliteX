...
class ClientBasicInfo(BaseModel):
    ...
    communication_no_1: Optional[str] = Field(None, description="Primary communication number")
    communication_type_1: Optional[str] = Field(None, description="Type of primary communication (e.g., Mobile, Office)")
    communication_no_2: Optional[str] = Field(None, description="Secondary communication number")
    communication_type_2: Optional[str] = Field(None, description="Type of secondary communication")
    
    # Off-Us Relationships
    client_off_us_relationships: Optional[str] = Field(None, description="Other banking relationships client maintains")
    client_off_us_relationship_bank: Optional[str] = Field(None, description="Names of other banks client banks with")
...
class ManagerAgentOutput(BaseModel):
    """Complete structured output from Manager Agent"""
    

    
    # Client Profile
    client_info: ClientBasicInfo = Field(..., description="Basic client identification and profile")
    risk_profile: RiskProfile = Field(..., description="Client risk appetite and profile")
    segment_info: SegmentInfo = Field(..., description="Banking segment and tier")
    rm_info: RMInfo = Field(..., description="Relationship Manager assignment")
    financial_metrics: FinancialMetrics = Field(..., description="Core financial metrics")
    relationship_tenure_years: float = Field(..., description="Banking relationship tenure in years")
    kyc_expiry_date: Optional[date] = Field(None, description="KYC expiry date")
    product_maturing_within_6m: List[str] = Field(..., description="Products maturing within 6 months")
    # AECB Alerts
    aecb_alerts: List[AECBAlert] = Field(..., description="Credit bureau alerts and their implications")

    
    # Growth Opportunities
    opportunities: List[OpportunityCategory] = Field(
        ..., description="Growth opportunities identified through share-of-potential analysis"
    )
    
    # Behavioral Insights
    behavioral_insights: List[BehavioralInsight] = Field(
        ..., description="Transaction patterns and behavioral indicators"
    )
    
    # Portfolio Overview
    portfolio_allocation: PortfolioAllocation = Field(..., description="Current investment portfolio allocation")
    deposit_trend: DepositTrend = Field(..., description="CASA deposit trend analysis")
    
    # Engagement Profile
    engagement_profile: EngagementProfile = Field(..., description="Client engagement and communication profile")

    # Immediate Actions
    immediate_actions: List[ImmediateActionItem] = Field(
        ..., description="Time-sensitive action items requiring RM attention"
    )
    
    # Summary for Downstream Agents
    downstream_summary: str = Field(
        ..., 
        description="Summary highlighting key opportunities and readiness for downstream agents"
    )
    
    # Data Sources
    data_sources_used: List[str] = Field(..., description="List of all tools/tables used for this analysis")
...
class AssetAllocationAgentOutput(BaseModel):
    ...
    @field_validator('current_total_aum_aed', 'total_rebalancing_amount_aed', mode='before')
    @classmethod
    def parse_decimal_fields(cls, v):
        return parse_decimal_field(v)
...
class InvestmentAgentOutput(BaseModel):
    """Complete structured output from Investment Agent"""
    
    client_id: str = Field(..., description="Client identifier")
    RM_id: str = Field(..., description="Relationship Manager ID")
   

    # Opportunity Summary
    opportunity_summary: InvestmentOpportunitySummary = Field(
        ..., description="Summary of investment growth opportunities"
    )
    ...
    product_recommendations: List[InvestmentProductRecommendation] = Field(
        ...
        min_items=1,
        max_items=3,
        description="Top 3 investment product recommendations with 5-step justification"
    )
    
    # Data Sources
    data_sources_used: List[str] = Field(..., description="List of all data used for investment analysis")
...
class BankingAgentOutput(BaseModel):
    """Complete structured output from Banking/CASA Agent"""
    
    client_id: str = Field(..., description="Client identifier")
    
    # Banking Relationship Overview
    casa_accounts: List[CASAAccountDetail] = Field(..., description="CASA account details")
    total_casa_balance_aed: Decimal = Field(..., description="Total CASA balance in AED")
    
    
    # Transaction Activity
    recent_transactions: List[BankingTransaction] = Field(..., description="Recent notable transactions")
    
    
    # Liquidity Assessment
    liquidity_position: str = Field(..., description="Current liquidity position assessment")
    ...
    product_recommendations: List[BankingProductRecommendation] = Field(
        ...
        min_items=1,
        max_items=3,
        description="Top 3 banking product recommendations with 5-step justification"
    )
    
    # Data Sources
    data_sources_used: List[str] = Field(..., description="List of all tools/tables used for banking analysis")
...
class BancassuranceAgentOutput(BaseModel):
    ...
    product_recommendations: List[BancassuranceProductRecommendation] = Field(
        ...
        min_items=1,
        max_items=5,
        description="Top 3-5 bancassurance recommendations with 5-step justification"
    )
...
class RMStrategyAgentOutput(BaseModel):
    ...
    preferred_channel: Optional[str] = Field(None, description="Preferred communication channel")
    communication_tone: str = Field(..., description="Recommended communication tone")
    meeting_agenda: List[str] = Field(..., description="Suggested meeting agenda structure")
    
    # Risk Flags and Mitigation
    risk_flags: List[RiskFlag] = Field(..., description="Risk flags requiring attention")
    
    # Success Metrics
    success_metrics: List[SuccessMetric] = Field(..., description="Success metrics for tracking")
    
    # Follow-up Schedule
    immediate_followup: List[str] = Field(..., description="Immediate follow-up items (24-48 hours)")
    weekly_followup: List[str] = Field(..., description="This week follow-up items (7 days)")
    monthly_followup: List[str] = Field(..., description="This month follow-up items (30 days)")
    quarterly_review: List[str] = Field(..., description="Quarterly review items")
    
    
    # Agent Outputs Summary
    manager_output_summary: str = Field(..., description="Key points from Manager Agent")
    risk_output_summary: str = Field(..., description="Key points from Risk Agent")
    investment_output_summary: str = Field(..., description="Key points from Investment Agent")
    loan_output_summary: str = Field(..., description="Key points from Loan Agent")
    banking_output_summary: str = Field(..., description="Key points from Banking Agent")
    bancassurance_output_summary: str = Field(..., description="Key points from Bancassurance Agent")
...
class MarketIntelligenceAgentOutput(BaseModel):
    """Structured output for Market Intelligence Agent - provides market context and economic insights"""
    
    # Market Overview
    market_overview: str = Field(description="Current market conditions and key trends")
    
    # Economic Indicators
    economic_indicators: Dict[str, Any] = Field(description="Key economic indicators relevant to client portfolio")
    
    # Sector Performance
    sector_performance: Dict[str, Any] = Field(description="Performance analysis by sector")
    
    # Risk Scenarios
    risk_scenarios: List[Dict[str, Any]] = Field(description="Market risk scenarios and their potential impact")
    
    # Investment Themes
    investment_themes: List[str] = Field(description="Current investment themes and opportunities")
    
    # Market Timing Insights
    timing_insights: str = Field(description="Market timing insights for investment decisions")
    
    # Currency and Commodity Trends
    currency_commodity_trends: Dict[str, Any] = Field(description="Currency and commodity market trends")
    
    # Regional Market Analysis
    regional_analysis: Dict[str, Any] = Field(description="Regional market performance and outlook")
    
    # Agent_Recommends
    Agent_Recommends: str = Field(default="", description="Executive summary of market intelligence insights, actionable and data-driven.")
